...
Things are actually even simpler over in `new-horizons` on the `pluto` branch. I don’t have to change branches at all; I can just rebase on top of `master` from `origin` directly:

```sh
$ git pull --rebase origin master
```

The result looks like this: <!-- TODO: the above is not all that useful, because this was half-rewritten, apparently? :sigh: I need to finish rewriting the piece (probably with diagrams work happening *in parallel*), integrating what is good from each of these flows. -->
...
Now, my commit graph for `pluto` is just `master` with one extra commit, the upgrade:
