...
In the tradition of old-school blogging,[^blogging] I thought I would respond… publicly! I am going to take these in reverse order, because on the tradeoff she highlights, I largely agree; while on the value provided by a conversion, I both sympathize and also think there is more to say.
...
Those kinds of real product changes go further when you include the kinds of work we actually have done to make it possible to get type safety in our libraries for capabilities like internationalization. I think untranslated strings are a big deal for our members! And from an internal perspective, the same goes for our tracking libraries: if your A–B test data is not reliable, it is very hard to know whether a given experimental feature is paying for itself or not!
...
But let us grant the basic claim for a moment, because there are times when converting a file required some ingenuity but did genuinely leave all the end-user functionality unchanged. I still do not think we have “achieved basically in reality” in those cases.

First, when Rach describes the “real dopamine rush from successfully converting a gnarly <abbr title="JavaScript">JS</abbr> file into a <abbr title="TypeScript">TS</abbr> one”, she indirectly highlights the reality that in many cases the complexity was already present in the code base. The TypeScript conversion did not create that complexity: It exposed it. Real-world JavaScript code is often incredibly complicated and indeed “clever” in ways that only become obvious when we try to express in types the contracts the code already invisibly assumes. My experience is that conversions from JavaScript require complex code far more often than does authoring code in TypeScript in the first place. I think the reason for this experience is simple: a great deal of that complexity is implicit in JavaScript, while writing out the contracts in TypeScript makes the complexity explicit. That enables better choices: does this particular <abbr title="application programming interface">API</abbr> actually warrant some complicated types, or should we just keep it simple? (Usually: the latter!)

Second, and building on the first, I often think of [a comment by Mark Seeman][seeman] on exactly this theme, though at a far more general level (emphasis his, strong emphasis mine):
...
> 
> Code quality is when you care about the readability and malleability of the code. **It's when you care about the code's ability to *sustain* the business, not only today, but also in the future. ... At the intersection between software and code quality the code sustains the business both now, and in the future.**
> 
...
Even a TypeScript conversion which leaves *all* the end-user functionality exactly as it was before can meaningfully improve the sustainability of the code base. (“Can”, I say, not “does”, because the details always matter. Just the same holds for *any* code we write, though.) As Rach herself put it:

> The part I like most is being able to “see what things are”. Now I get frustrated when I hover over a function or variable in VSCode, expecting it to tell me its types, and it can’t because it was imported from a JS file.

For just the same reasons, it makes navigating the code base easier, because go-to-definition *just works*, everywhere, all the time, instead of being driven by a fragile pile of heuristics which are often wrong.[^ls] It also makes changing the code later easier. For one thing, automated end-to-end refactoring becomes possible in a way it simply cannot be without types. Even one’s ability to make non-automatable transformations is improved: being able to accurately and instantaneously find all references to a given <abbr title="application programming interface">API</abbr> makes it far easier to design a sweeping change involving that code.
